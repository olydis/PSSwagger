...
function New-SwaggerPath {
    ...
    $tsResult = Eval-Ts $tsSwaggerUtils "doParameterStuff" $AsJobParameterString, $clientName, $swaggerDict["Security"], $swaggerDict["SecurityDefinitions"], $swaggerDict["Info"], (ConvertTo-PsCustomObjectFromHashtable $swaggerDict["Definitions"]), $swaggerDict["CommandDefaults"], $isNextPageOperation, $UseAzureCsharpGenerator.IsPresent, $SwaggerMetaDict['PowerShellCodeGen'], $DefinitionFunctionsDetails, $PathFunctionDetails, $FunctionDetails
    $paramHelp = $tsResult.paramHelp
    $DefaultParameterSetName = $tsResult.defaultParameterSetName
    $commandHelp = $tsResult.commandHelp
    $ParamBlockReplaceStr = $tsResult.paramBlockReplaceStr
    $PathFunctionBody = $tsResult.pathFunctionBody
    $bodyObject = $tsResult.bodyObject
    $PSCmdletOutputItemType = $tsResult.psCmdletOutputItemType
