...
function getPascalCasedString(name) {
    if (name) {
...
// pluralization
const english_pluralization_service_1 = require("C:/work/english-pluralization-service");
function getPluralizationService() {
    const result = new english_pluralization_service_1.EnglishPluralizationService();
...
function getSingularizedValue(name) {
    return getPascalCasedString(singularize(name));
}
// verb stuff
const cmdVerbMap_GetVerb = {
    "Add": "Common",
    "Clear": "Common",
    "Close": "Common",
    "Copy": "Common",
...
function ExistsVerb(verb) {
    return MapVerb(verb).length > 0;
}
// 
function getPsTypeFromSwaggerObject(parameterType, jsonObject) {
...
function getPathCommandName(operationId) {
        ...
        if (ExistsVerb(cmdVerb)) {
            ...
            if (-1 !== beginningOfSuffix) {
                ...
                if (!cmdNoun) {
                    ...
                    if (!cmdNoun.toLowerCase().includes(cmdNounSuffix.toLowerCase()) &&
